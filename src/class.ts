      ...
      for (const { descendants, values } of this.#inheritances)
        for (const descendant of Array.isArray(descendants) ? descendants : [descendants]) {
          ...
          for (const [key, value] of data) {
            const chain: Record<string, P> = propsChain.get(descendant.#Id) ?? {}

            if (!(key in chain) || !propsChain.has(descendant.#Id)) {
              propsChain.set(descendant.#Id, { ...chain, [key]: value })
    ...
    if (elements)
      for (const element of elements) {
        if (element instanceof Element && element.#getTagName() === 'w-slot') {
          if (this.#slot) {
            const slotName: string | Element<D, P> = this.#convertHtml(element.#html)?.[0] ?? ''
  ...
  setData(key: keyof D, value: D[typeof key]): void {
    if (!(key in this.#data)) throw Error(`${key as string} is not defined in data...`)
      ...
      for (const { setProps } of this.#propsTrees.filter(
        tree => tree.ancestorId === this.#Id && tree.dataKey === key
      ))
